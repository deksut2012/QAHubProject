...
{
 public static IEndpointRouteBuilder MapBugEndpoints(this IEndpointRouteBuilder endpoints){var g=endpoints.MapGroup("/api/v1/bugs").WithTags("Bugs").RequireAuthorization(AuthorizationPolicies.ProductAccess);g.MapGet("/",GetAll);g.MapGet("/{id:guid}",Get);g.MapPost("/",Create);g.MapPost("/{id:guid}/transition",Transition);return endpoints;}
 ...
 private static async Task<IResult> Transition(Guid id,TransitionBugRequest r,ClaimsPrincipal user,QAHubDbContext db,CancellationToken ct)
 {
  ...
  try{bug.TransitionTo(r.Status,user.Identity?.Name??"unknown",r.Reason??"",r.FixBuildId,r.CanonicalBugId);await db.SaveChangesAsync(ct);return Results.Ok(Map(bug));}catch(InvalidOperationException ex){return Results.Conflict(new{message=ex.Message});}
 }
 private static IQueryable<Bug> Query(QAHubDbContext db)=>db.Bugs.Include(x=>x.RunLinks).Include(x=>x.History).AsSplitQuery();
 private static BugResponse Map(Bug x)=>new(x.Id,x.ProductId,x.Code,x.Title,x.Description,x.StepsToReproduce,x.ExpectedResult,x.ActualResult,x.Severity,x.Priority,x.Status,x.Reporter,x.Assignee,x.FixBuildId,x.CanonicalBugId,x.CreatedAtUtc,x.UpdatedAtUtc,Math.Max(0,(int)(DateTimeOffset.UtcNow-(x.ClosedAtUtc??x.CreatedAtUtc)).TotalDays),x.RunLinks.Select(l=>l.TestRunAttemptId).ToList(),x.History.OrderBy(h=>h.ChangedAtUtc).Select(h=>new BugHistoryResponse(h.FromStatus,h.ToStatus,h.ActorId,h.Reason,h.ChangedAtUtc)).ToList());
