...
public sealed class Bug
{
    ...
    public void TransitionTo(BugStatus next,string actor,string reason,Guid? fixBuildId=null,Guid? canonicalBugId=null,Guid? verificationAttemptId=null)
    {
        ...
        if(next==BugStatus.Verified&&!verificationAttemptId.HasValue)throw new InvalidOperationException("Verified bug requires a passed retest attempt.");
        var previous=Status;Status=next;if(fixBuildId.HasValue)FixBuildId=fixBuildId;if(canonicalBugId.HasValue)CanonicalBugId=canonicalBugId;if(verificationAttemptId.HasValue)VerificationAttemptId=verificationAttemptId;UpdatedAtUtc=DateTimeOffset.UtcNow;if(next==BugStatus.Closed)ClosedAtUtc=UpdatedAtUtc;History.Add(new BugStatusHistory(Id,previous,next,actor,reason));
    }
}
